...
classifies results with Claude, stores in SQLite, and exports
site/data/qa.json.

As of the sansad_client.py rewrite, search + detail extraction happens in
ONE page load per keyword×house (the site expands result rows in place
rather than linking to separate detail pages) — much cheaper than the
original two-phase design, but this site is still confirmed slow
(2-3+ minutes per page load), so KEYWORDS_PER_RUN is still deliberately
conservative. Increase it once you've confirmed a run completes cleanly.

Unlike bills (stable once passed), a Q&A entry can start unanswered and
get its answer added later — only entries that are already answered (or
manually corrected) are treated as finished; anything still awaiting an
answer gets re-checked on every run.
"""
...
# One page load now covers a whole keyword's search + all its results'
# details (see sansad_client.py), so this can be a bit more generous than
# it was under the old one-load-per-result design — but the site is still
# slow, so start here and raise it once you've watched a run succeed.
KEYWORDS_PER_RUN = 4
...
def export_json():
    ...
    # See scraper/update_bills.py for why this guard exists — Next.js's
    # static export misreports an empty generateStaticParams() array as a
    # build error, so a legitimately-empty run must never export [].
    if not entries:
        entries = [{
