...
const SEGMENTS: { key: keyof Bill; label: string; max: number; color: string }[] = [
  ...
  { key: "enforceability_score", label: "Enforceability", max: 20, color: "#f16223" },
  { key: "scale_score", label: "Scale of impact", max: 15, color: "#009ed8" },
  { key: "novelty_score", label: "Novelty", max: 10, color: "#B0B0B0" },
];

const TOTAL_MAX = SEGMENTS.reduce((sum, s) => sum + s.max, 0);

export function ScoreBar({ bill, compact = false }: { bill: Bill; compact?: boolean }) {
  if (bill.total_score === null) {
    return <div className="font-mono text-xs text-inkmuted italic">not yet scored</div>;
  }
      ...
      <div className={`flex w-full rounded-sm overflow-hidden ${compact ? "h-2" : "h-3"} bg-rule`}>
        {SEGMENTS.map((seg) => {
          ...
          return (
            ...
            >
              <div className="h-full" style={{ width: `${fillPct}%`, backgroundColor: seg.color }} />
            </div>
          );
        })}
      </div>
